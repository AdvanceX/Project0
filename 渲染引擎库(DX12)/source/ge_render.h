...
class CShader{
	...
	open UINT CreateInputDesc(UINT Slot, SharedPtr<D3DInputElemDesc> &Output){
...
class CGfxPso{
	...
	open VOID Initialize(ID3DDevice* lpD3DDevice, CShader &Shader, GEWnd &Window){
		...
		SharedPtr<D3DInputElemDesc> InputDesc;
...
struct Mirror{
	...
	static VOID BuildReflectionMatrix(VECTOR3 &Plane, MATRIX &matOut){
...
//阴影
struct Shadow{
	static VOID BuildShadowMatrix(VECTOR3 &Plane, VECTOR4 &Light, MATRIX &matOut){

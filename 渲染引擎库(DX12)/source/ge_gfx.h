...
class CCamera{
	...
	open CCamera(const VECTOR3 &Position, SPFP FOVAngle, SPFP AspectRatio, SPFP NearZ, SPFP FarZ){
		self.Initialize(Position, FOVAngle, AspectRatio, NearZ, FarZ);
	}
	//预置(位置,视场角,投影宽高比,近裁剪面,远裁剪面)
	open VOID Initialize(const VECTOR3 &Position, SPFP FOVAngle, SPFP AspectRatio, SPFP NearZ, SPFP FarZ){
	...
	//观测(位置,目标,上方向)
	open VOID LookAt(const VECTOR3 &Position, const VECTOR3 &Target, const VECTOR3 &vecUp){
		self.Position = PackV3(Position);
...
class CVertView{
	...
	open VOID Initialize(DWRD Format){
		...
		if(Format & VERT_NORMAL){
			FORCE_CVT(self.hNormal, self.ByteCount);
			self.ByteCount += sizeof(VECTOR3);
		}
		if(Format & VERT_TANGENT){
			FORCE_CVT(self.hTangent, self.ByteCount);
			self.ByteCount += sizeof(VECTOR3);
		}
		if(Format & VERT_SIZE){
			FORCE_CVT(self.hPointSize, self.ByteCount);
			self.ByteCount += sizeof(VECTOR2);
		}
		if(Format & VERT_BONES){
			FORCE_CVT(self.hWeights, self.ByteCount);
			self.ByteCount += sizeof(SPFP) * 3;
			FORCE_CVT(self.hBoneIndexes, self.ByteCount);
	...
	open VOID SetAddress(const VOID* Address){
...
class CAnimation{
	...
	open VOID Initialize(const TCHR* FilePath){
		HANDLE hFile = FindFileChunk(FilePath, ASSET_ANIM);
	...
	open VOID UpdateBones(SPFP Time, BONE* arrBone, BOOL bLoop = TRUE){
		...
		for(UINT i = 0; i < self.numSubset; ++i){
			if(pAnim->numFrame > 0)
				pAnim->Interpolate(Time, arrBone[i].matTrans);
			pAnim++;
		}
	}
};
...
class CMorphAnim{
	...
	open VOID Interpolate(SPFP Time, UINT Targets[2], SPFP &Lerp){
		UINT LastFrame = self.numFrame - 1;
		MORPHFRAME *Frames = self.arrFrame;

		if(Time <= Frames[0].Time){
			Targets[0] = Targets[1] = Frames[0].Target;
		} else if(Time >= Frames[LastFrame].Time){
			Targets[0] = Targets[1] = Frames[LastFrame].Target;
		} else{
			MORPHFRAME *F0 = &Frames[0];
			MORPHFRAME *F1 = &Frames[1];
...
struct CSoftBody{
	...
	CSoftBody(BYTE *VtxBuffer, BYTE *IdxBuffer, UINT cbVertex, UINT VtxCount, UINT IdxCount, SPFP Mass, SPFP KS, SPFP KD){
		self.Initialize(VtxBuffer, IdxBuffer, cbVertex, VtxCount, IdxCount, Mass, KS, KD);
	}

	VOID Initialize(BYTE *VtxBuffer, BYTE *IdxBuffer, UINT cbVertex, UINT VtxCount, UINT IdxCount, SPFP Mass, SPFP KS, SPFP KD){
		SPFP RM;
		BYTE *pByte;
		USRT P0, P1, P2;
		XVECTOR Coord;
		...
		CopyMemory(self.arrFace, IdxBuffer, sizeof(INDEX3) * self.numFace);

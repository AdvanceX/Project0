...
struct Label: public CtrlEx{
	...
	Label(Label &label){
		*this = label;
		self.caption = new WCHR[WcsLen(label.caption) + 1];
		WcsCpy(label.caption, self.caption);
	}
	...
	VOID InitAsSample(WCHR* caption, IDWTxtFormat* lpTxtFormat, ID2DBrush* lpTxtBrush){
		...
		self.caption = new WCHR[WcsLen(caption) + 1];
		WcsCpy(caption, self.caption);
	}
	...
	VOID OnRender(){
		D2DRectF rcFill;
		self.GetFillRect(rcFill);
		self.GetD2DDC()->DrawText(self.caption, (UINT)WcsLen(self.caption),
...
struct TxtBox: public CtrlEx{
	...
	VOID SetText(WCHR* content){
		...
		self.SetCaretPos((INT4)WcsLen(content));
	}
	VOID InsertText(WCHR* content){
		self.content.insert(self.iCharIndex, content);
		self.SetCaretPos(self.iCharIndex + (INT4)WcsLen(content));
	}
	...
	VOID SetCaretPos(INT4 position){
		if((position >= 0) && (position <= self.content.length())){
			CStringW string = self.content.substr(0, position);
	...
	VOID OnRender(){
		...
		if(self.fstItem){
			...
			for(UINT i = 0; i < self.nItemFrame; ++i){
				...
				lpD2DDC->DrawText(pItem->caption, (UINT)WcsLen(pItem->caption),
					self.lpTxtFormat, rcFill, self.lpTxtBrush);

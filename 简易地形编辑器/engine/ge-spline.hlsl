...
//常量外壳着色
HSPatchTess ConstHullShading(InputPatch<CPoint, 4> Patch, uint PatchId: SV_PrimitiveID){
	HSPatchTess PatchTess;
...
//域着色
[domain("quad")]
DSOutput DomainShading(const OutputPatch<CPoint, 4> Patch, HSPatchTess PatchTess, float2 Location: SV_DomainLocation){
...
float4 PixelShading(GSOutput Input): SV_Target{
	...
	GMinMtl MtlBase = { Color.rgb, gRoughness, gMetalness };

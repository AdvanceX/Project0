...
#include "../library/AwesomeCpp/cJSON.h"
#include "../engine/ge-base.hpp"
#include "../engine/ge-util.hpp"

#ifdef _WIN64
#pragma comment(lib, "library/AwesomeCpp/x64/cJSON.lib")
#else
#pragma comment(lib, "library/AwesomeCpp/x86/cJSON.lib")
#endif
...
struct WColor4{
	...
	operator COLORREF(){
		...
		UNS32 R = UNS32($m.R * kRatio);
		UNS32 G = UNS32($m.G * kRatio);
		UNS32 B = UNS32($m.B * kRatio);
		UNS32 A = UNS32($m.A * kRatio);
...
struct GMtlBase{
	...
	GMtlBase(): Albedo(1.f, 1.f, 1.f), Opacity(1.f),
...
struct GBone{
	...
	GBone() = default;
	GBone(UNS32 Level, UNS32 iParent, GNode *_in pNode){
		$m.Level = Level;
		$m.iParent = iParent;
	...
	static bool IsLess(_in GBone &Bone0, _in GBone &Bone1){
		if(Bone0.Level < Bone1.Level)
			return B_True;
		if(Bone0.Level > Bone1.Level)
			return B_False;
		if(strcmp(Bone0.psName, Bone1.psName) < 0)
			return B_True;
		return B_False;
	}
};
...
class CModel{
	...
	_secr $VOID InitIndexBuffer(CJsonNode*, _in GBufView*);
	_secr $VOID InitVertexBuffer(CJsonNode*, _in GBufView*);
};

// Morphs
class CMorphs{
	_open UNS32 Ident;
	_open UNS32 cbTarget;
	_open UNS32 numTarget;
	_open XMFLOAT3 **lprgTarget;

	_open ~CMorphs(){
		for(UNS32 N = 0; N < $m.numTarget; ++N)
			SAFE_DELETEA($m.lprgTarget[N]);
...
class CAnimation{
	...
	_open UNS32 numTrans;
	_open UNS32 numScaling;
	_open UNS32 numRotation;
	_open GRotFrame *lprgRotation;
	_open GAnimFrame *lprgScaling;
	_open GAnimFrame *lprgTrans;
...
class CGltfParser{
	...
	_secr UNS32 numMatl;
	_secr UNS32 numNode;
	_secr UNS32 numModel;
	_secr UNS32 numBuffer;
	_secr UNS32 numBufView;
	_secr UNS32 numTexture;
	///缓存
	_secr UNS32 rgBoneRig[256];
	_secr GBone *lprgBone;
	_secr GNode *lprgNode;
	_secr BYTET **lprgBuffer;
	_secr CModel *lprgModel;
	_secr GBufView *lprgBufView;
	_secr GTexView *lprgTexView;
	_secr GMaterial *lprgMatl;
	_secr CAnimAssy *lprgAnim;

	_open ~CGltfParser(){
		for(UNS32 N = 0; N < $m.numBuffer; ++N)
			delete[] $m.lprgBuffer[N];
	...
	_secr $VOID ParseMaterial(CJsonNode*, _out GMaterial&);
	_secr $VOID ParseSkeleton(_in GNode*);
	...
	_secr $VOID EndChunk(HANDLE, _io UNS32&);
	_secr BYTET *GetBuffer(UNS32);
	_secr UNS32 GetBufferSplit(UNS32);

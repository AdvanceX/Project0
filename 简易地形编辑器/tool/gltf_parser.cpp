...
UNS32 StringHash(_in CHAR8 *psInput){
...
$VOID ExitProgram(_in CHAR8 *psPrompt, ...){
	...
	vsnprintf(Text, BUF_SIZE, psPrompt, pArgs);
	va_end(pArgs);
...
$VOID CAnimAssy::Initialize(CJsonNode *pDomAnim, CGltfParser &Parser){
	...
	while(pDomChan){
		...
		UNS32 NodeId = pDomTarget->GetMemI("node");
		UNS32 BoneId = Parser.lprgNode[NodeId].iRefBone;
...
$VOID CAnimation::Initialize(_in CHAR8 *psType, CJsonNode *pDomSmpl, CGltfParser &Parser){
	UNS32 BuffId0 = pDomSmpl->GetMemI("input");
	UNS32 BuffId1 = pDomSmpl->GetMemI("output");
	UNS32 FrameCnt = Parser.GetBufferSplit(BuffId0);
	...
	if(!strcmp(psType, "rotation")){
		$m.numRotation = FrameCnt;
		$m.lprgRotation = new GRotFrame[FrameCnt];

		for(UNS32 iFrame = 0; iFrame < FrameCnt; ++iFrame){
			$m.lprgRotation[iFrame].Time = prgTime[iFrame];
			$m.lprgRotation[iFrame].Xform = prgRotation[iFrame];
		}
	} else if(!strcmp(psType, "scale")){
		$m.numScaling = FrameCnt;
		$m.lprgScaling = new GAnimFrame[FrameCnt];

		for(UNS32 iFrame = 0; iFrame < FrameCnt; ++iFrame){
			$m.lprgScaling[iFrame].Time = prgTime[iFrame];
			$m.lprgScaling[iFrame].Xform = prgTxform[iFrame];
		}
	} else if(!strcmp(psType, "translation")){
		$m.numTrans = FrameCnt;
		$m.lprgTrans = new GAnimFrame[FrameCnt];

		for(UNS32 iFrame = 0; iFrame < FrameCnt; ++iFrame){
			$m.lprgTrans[iFrame].Time = prgTime[iFrame];
			$m.lprgTrans[iFrame].Xform = prgTxform[iFrame];
		}
	}
...
$VOID CModel::InitVertexs(CJsonNode *pDomPrims, CGltfParser &Parser){
	...
	UNS32 AttrCnt = _ARRAYSIZE(rgAttrId);
	CJsonNode *pDomPrim = pDomPrims->GetChild();

	while(pDomPrim){
		CJsonNode *pDomAttrs = pDomPrim->GetChild("attributes");
		UNS32 BufIndex = pDomAttrs->GetMemI("POSITION");
		UNS32 VtxCount = Parser.GetBufferSplit(BufIndex);

		for(UNS32 iAttr = 0; iAttr < AttrCnt; ++iAttr){
			...
			/*if(!strcmp(psAttrId, "POSITION") || !strcmp(psAttrId, "NORMAL")){
				for(UNS32 iVtx = 0; iVtx < VtxCount; ++iVtx){
			...
			} else */if(!strcmp(psAttrId, "COLOR_0")){
				for(UNS32 iVtx = 0; iVtx < VtxCount; ++iVtx){
					*(COLORREF*)psDstData = *(WColor4*)psSrcData;
					psSrcData += sizeof(WColor4);
					psDstData += VtxView.cbVertex;
				}
			} else if(!strcmp(psAttrId, "JOINTS_0")){
				for(UNS32 iVtx = 0; iVtx < VtxCount; ++iVtx){
			...
			} else if(!strcmp(psAttrId, "WEIGHTS_0")){
				for(UNS32 iVtx = 0; iVtx < VtxCount; ++iVtx){
					Mem_Move(psDstData, psSrcData, rgStride[iAttr]);
...
$VOID CModel::InitSubmeshs(CJsonNode *pDomPrims, CGltfParser &Parser){
	...
	GSubMesh *pSubmesh = $m.lprgSubset;
	GMaterial *prgMatl = Parser.lprgMatl;
	CJsonNode *pDomPrim = pDomPrims->GetChild();
	...
	while(pDomPrim){
		...
		UNS32 IbvIndex = pDomIbv->GetValI();
		UNS32 MtlIndex = pDomMatl ? pDomMatl->GetValI() : UINT_MAX;

		BYTET *psIdxSrc = Parser.GetBuffer(IbvIndex);
		GBufView *pIBView = Parser.lprgBufView + IbvIndex;

		Mem_Move(psIdxDest, psIdxSrc, pIBView->BufSize);
...
$VOID CModel::InitIndexBuffer(CJsonNode *pDomPrims, _in GBufView *prgBufView){
	UNS32 ViewId = 0, BufSize = 0;
	CJsonNode *pDomPrim = pDomPrims->GetChild();
...
$VOID CModel::InitVertexBuffer(CJsonNode *pDomPrims, _in GBufView *prgBufView){
	//顶点格式
	...
	UNS32 ViewId, BufSize = 0;
	UNS32 VtxStride = GVtxView(VtxFormat).cbVertex;
...
$VOID CGltfParser::Process(_in CHAR8 *psSrcPath){
	...
	WCHAR *psSrcPathW = MultiByteToWideChar(psSrcPath);
	CJsonNode *lpDomRoot = CJsonNode::CreateByFile(psSrcPathW);

	strcpy(DstPath, psSrcPath);
	strcpy(strrchr(DstPath, '.'), ".dat");
...
$VOID CGltfParser::ParseSkin(CJsonNode *pDomRoot){
	...
	UNS32 iMatBuff = pDomSkin->GetMemI("inverseBindMatrices");
	XMFLOAT4X4 *rgInvMat = (XMFLOAT4X4*)$m.GetBuffer(iMatBuff);

	$m.ParseSkeleton(&$m.lprgNode[pDomJoint->GetValI()]);

	for(UNS32 iJoint = 0; pDomJoint; ++iJoint){
		UNS32 iNode = pDomJoint->GetValI();
		UNS32 iBone = $m.lprgNode[iNode].iRefBone;
...
$VOID CGltfParser::ParseNodes(CJsonNode *pDomRoot){
	...
	$m.lprgNode = new GNode[$m.numNode];
...
$VOID CGltfParser::ParseBuffers(CJsonNode *pDomRoot){
	...
	while(pDomBuff){
		...
		*ppBuffer = new CHAR8[BufSize];
		...
		hrSrcFile = CreateFile2(MultiByteToWideChar(psSrcUri), GENERIC_READ, 0L, OPEN_EXISTING, P_Null);
...
$VOID CGltfParser::ParseTextures(CJsonNode *pDomRoot){
	...
	$m.lprgTexView = new GTexView[$m.numTexture];

	for(UNS32 iTex = 0; iTex < $m.numTexture; ++iTex){
		...
		strcpy(ImgPath, pDomImg->GetMemS("uri"));
		*strrchr(ImgPath, '.') = '\0';

		psImgName = strrchr(ImgPath, '/');
...
$VOID CGltfParser::ParseMaterials(CJsonNode *pDomRoot){
	...
	$m.lprgMatl = new GMaterial[$m.numMatl];
...
$VOID CGltfParser::ParseBufferViews(CJsonNode *pDomRoot){
	...
	$m.lprgBufView = new GBufView[$m.numBufView];
...
$VOID CGltfParser::ParseMaterial(CJsonNode *pDomMatl, _out GMaterial &rMatl){
	CJsonNode *pDomPbr;
	CJsonNode *pDomProp;

	GMtlBase &rBase = rMatl.Base;
	GMtlMaps &rMaps = rMatl.Maps;
	...
	if(pDomProp = pDomPbr->GetChild("baseColorTexture")){
		GTexView *pView = $m.lprgTexView + pDomProp->GetMemI("index");
...
$VOID CGltfParser::ParseSkeleton(_in GNode *pRoot){
	//查找所有骨骼节点
	...
	TSeqList<GBone> BoneList;
	TDEQueue<SNodeEx> NodeQue;

	NodeQue.push_back({ 0, UINT_MAX, (GNode*)pRoot });
	while(!NodeQue.empty()){
		SNodeEx Node = NodeQue.front();
		SNodeEx SubNode = { Node.Level + 1, OBJ_PID(Node.pNode) };
		GBone Bone = { Node.Level, Node.iParent, Node.pNode };

		NodeQue.pop_front();
		BoneList.push_back(Bone);

		for(UNS32 iChild = 0; iChild < Node.pNode->numChild; ++iChild){
			UNS32 iNode = Node.pNode->rgChild[iChild];
	...
	std::sort(BoneList.begin(), BoneList.end(), GBone::IsLess);

	//保存骨骼

	TTreeMap<UNS32, UNS32> mapIdToSeq;
	mapIdToSeq.insert({ OBJ_ID(BoneList[0]), 0 });

	$m.numBone = (UNS32)BoneList.size();
	$m.lprgBone = new GBone[$m.numBone];

	$m.lprgBone[0] = BoneList[0];
	$m.lprgBone[0].pRefNode->iRefBone = 0;

	for(UNS32 iBone = 1; iBone < $m.numBone; ++iBone){
		GBone *pBone = $m.lprgBone + iBone;
...
$VOID CGltfParser::WriteModels(HANDLE hFile){
	WriteFile(hFile, &$m.numModel, sizeof(UNS32), P_Null, P_Null);

	for(UNS32 iMdl = 0; iMdl < $m.numModel; ++iMdl){
		CModel &rModel = $m.lprgModel[iMdl];
		UNS32 NameLen = (UNS32)strlen(rModel.psName);

		WriteFile(hFile, &NameLen, sizeof(UNS32), P_Null, P_Null);
		...
		WriteFile(hFile, &rModel.cbIdxBuff, sizeof(UNS32), P_Null, P_Null);
		WriteFile(hFile, &rModel.cbVtxBuff, sizeof(UNS32), P_Null, P_Null);
		WriteFile(hFile, &rModel.numSubset, sizeof(UNS32), P_Null, P_Null);
		...
		if(rModel.numSubset > 0){
			UNS32 BufSize = rModel.numSubset * sizeof(GSubMesh);
...
$VOID CGltfParser::WriteSkeleton(HANDLE hFile){
	WriteFile(hFile, &$m.numBone, sizeof(UNS32), P_Null, P_Null);

	for(UNS32 iBone = 0; iBone < $m.numBone; ++iBone){
		GBone &rBone = $m.lprgBone[iBone];
		UNS32 NameLen = (UNS32)strlen(rBone.psName);

		WriteFile(hFile, &NameLen, sizeof(UNS32), P_Null, P_Null);
		WriteFile(hFile, rBone.psName, NameLen, P_Null, P_Null);
		WriteFile(hFile, &rBone.iParent, sizeof(UNS32), P_Null, P_Null);
...
$VOID CGltfParser::WriteMaterials(HANDLE hFile){
	UNS32 BufSize = $m.numMatl * sizeof(GMaterial);
	WriteFile(hFile, &$m.numMatl, sizeof(UNS32), P_Null, P_Null);
...
$VOID CGltfParser::WriteAnimations(HANDLE hFile){
	...
	WriteFile(hFile, &pAssy->numElem, sizeof(UNS32), P_Null, P_Null);

	for(; pAnim != pEndAnim; ++pAnim){
		UNS32 BufSize0 = pAnim->numTrans * sizeof(GAnimFrame);
		UNS32 BufSize1 = pAnim->numScaling * sizeof(GAnimFrame);
		UNS32 BufSize2 = pAnim->numRotation * sizeof(GRotFrame);

		WriteFile(hFile, &pAnim->numTrans, sizeof(UNS32), P_Null, P_Null);
		WriteFile(hFile, &pAnim->numScaling, sizeof(UNS32), P_Null, P_Null);
		WriteFile(hFile, &pAnim->numRotation, sizeof(UNS32), P_Null, P_Null);
...
$VOID CGltfParser::StartChunk(HANDLE hFile, DWORD Type){
	GChunkHead Header = { Type, 0 };
	WriteFile(hFile, &Header, sizeof(GChunkHead), P_Null, P_Null);
}

$VOID CGltfParser::EndChunk(HANDLE hFile, _io UNS32 &rCursor){
	UNS32 ChunkEnd = GetFileSize(hFile, P_Null);
	UNS32 DataSize = ChunkEnd - rCursor - sizeof(GChunkHead);

	SetFilePointer(hFile, rCursor + sizeof(DWORD), P_Null, FILE_BEGIN);
	WriteFile(hFile, &DataSize, sizeof(UNS32), P_Null, P_Null);
	SetFilePointer(hFile, ChunkEnd, P_Null, FILE_BEGIN);

	rCursor = ChunkEnd; //下一个文件块
}

BYTET *CGltfParser::GetBuffer(UNS32 ViewId){
	GBufView *pView = $m.lprgBufView + ViewId;
	BYTET *psBuffer = $m.lprgBuffer[pView->BuffId];
	return psBuffer + pView->Offset;
}

UNS32 CGltfParser::GetBufferSplit(UNS32 ViewId){
...
INT32 main(INT32 numArg, CHAR8 **prgArg){
	for(INT32 iArg = 1; iArg < numArg; ++iArg){
		CGltfParser Parser;

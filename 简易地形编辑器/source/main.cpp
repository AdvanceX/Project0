...
LRESULT _CBACK WinProc(HWND hWindow, UNS32 MsgCode, WPARAM Param0, LPARAM Param1){
		...
		((MsgCode >= WM_MOUSEFIRST) && (MsgCode <= WM_MOUSELAST)))
	{
		if(gMyAppPage->OnMessage(MsgCode, Param0, Param1))
			return 0;
		if(ImGui_ImplWin32_WndProcHandler(hWindow, MsgCode, Param0, Param1))
			return 0;
...
INT32 _SYSC WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, PSTR psCmdLine, LONG32 ShowCmd){
	...
	UNS32 WndWidth = GetSystemMetrics(SM_CXSCREEN);
	UNS32 WndHeight = GetSystemMetrics(SM_CYSCREEN);

class CMGMaster: public IAppBase{
	...
	_open $VOID Initialize(){
		...
		$m.Camera.Reset(MXVECTOR{ 0.f, 0.f, -10.f, 0.f });
	...
	_open $VOID Paint(){
		...
		gMyRender.SetModelAttrs(dx::XMMatrixIdentity());
		gMyRender.SetBones($m.Bones.GetPosture());
		gMyRender.BeginDraw(gMyWndEx);
		gMyRender.ClearWindow(gMyWndEx, *(FColor4*)dx::g_XMOne.f);

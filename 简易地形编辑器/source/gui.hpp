...
$VOID _CBACK mgDrawClock(CWidget *pCtrl, _in FRect &rcDest){
	FRect rcInner = rcDest;
...
$VOID _CBACK mgDrawMinimap(CWidget *pCtrl, _in FRect &rcDest){
	GE2d::lpPureBrush->SetColor(D2DXColor(0.8f, 0.8f, 0.8f));
	GE2d::lpD2dDevCtx->FillRectangle(rcDest, GE2d::lpPureBrush);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.f, 0.f, 0.5f));
	GE2d::lpD2dDevCtx->DrawRectangle(rcDest, GE2d::lpPureBrush, 1.f);
}
$VOID _CBACK mgDrawButton0(CWidget *pCtrl, _in FRect &rcDest){
...
$VOID _CBACK mgDrawButton1(CWidget *pCtrl, _in FRect &rcDest){
	if(!pCtrl->IsFocused()){
...
$VOID _CBACK mgDrawButton2(CWidget *pCtrl, _in FRect &rcDest){
	D2DXColor BgColor0(0.4f, 0.4f, 0.8f);
	D2DXColor BgColor1(0.f, 0.12f, 0.7f);
	D2DXColor BgColor = pCtrl->IsFocused() ? BgColor0 : BgColor1;

	GE2d::lpPureBrush->SetColor(BgColor);
	GE2d::lpD2dDevCtx->FillRectangle(rcDest, GE2d::lpPureBrush);
	((CButton*)pCtrl)->DrawBackground(rcDest);
}
$VOID _CBACK mgDrawResInfo0(CWidget *pCtrl, _in FRect &rcDest){
	FRect rcPart0 = rcDest, rcPart1 = rcDest;
	rcPart0.left = rcPart0.right - pCtrl->SizeY;
	rcPart1.right = rcPart0.left + 2.f;
	rcPart1.bottom -= pCtrl->SizeY * 0.2f;

	GE2d::lpPureBrush->SetColor(D2DXColor(1.f, 1.f, 1.f));
	GE2d::lpD2dDevCtx->FillRectangle(rcPart0, GE2d::lpPureBrush);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.1f, 0.1f, 0.1f));
	GE2d::lpD2dDevCtx->DrawRectangle(rcPart0, GE2d::lpPureBrush);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.5f, 0.5f, 0.5f));
	GE2d::lpD2dDevCtx->FillRectangle(rcPart1, GE2d::lpPureBrush);
}
$VOID _CBACK mgDrawResInfo1(CWidget *pCtrl, _in FRect &rcDest){
	//条目底色

	GE2d::lpPureBrush->SetOpacity(0.6f);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.6f, 0.6f, 0.6f));
	GE2d::lpD2dDevCtx->FillRectangle(rcDest, GE2d::lpPureBrush);
...
$VOID _CBACK mgDrawResInfo2(CWidget *pCtrl, _in FRect &rcDest){
	FRect rcPart0 = rcDest, rcPart1 = rcDest;
...
$VOID _CBACK mgDrawMenuPanel(CWidget *pCtrl, _in FRect &rcDest){
	D2D1_ROUNDED_RECT rcDest0 = { rcDest, 6.f, 6.f };
	GE2d::lpPureBrush->SetColor(D2DXColor(0.14f, 0.16f, 0.18f));
...
$VOID _CBACK mgDrawEditPanel(CWidget *pCtrl, _in FRect &rcDest){
	DrawBitmap(GE2d::lpD2dDevCtx, mgGetImage("Px-Panel"), rcDest);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.1f, 0.1f, 0.1f));
	GE2d::lpD2dDevCtx->DrawRectangle(rcDest, GE2d::lpPureBrush, 2.f);
}
$VOID _CBACK mgDrawMainPanel(CWidget *pCtrl, _in FRect &rcDest){
	GE2d::lpPureBrush->SetColor(D2DXColor(0.6f, 0.62f, 0.66f));
	GE2d::lpD2dDevCtx->FillRectangle(rcDest, GE2d::lpPureBrush);
	GE2d::lpPureBrush->SetColor(D2DXColor(0.14f, 0.16f, 0.18f));
	GE2d::lpD2dDevCtx->DrawRectangle(rcDest, GE2d::lpPureBrush, 2.f);
}
$VOID _CBACK mgDrawListItem(CWidget *pCtrl, _in FRect &rcDest){
	DrawBitmap(GE2d::lpD2dDevCtx, mgGetImage("Px-ListItem"), rcDest);
	CListBox::DrawDefText(pCtrl, rcDest);
}
$VOID _CBACK mgDrawListFocus(CWidget *pCtrl, _in FRect &rcDest){
	DrawBitmap(GE2d::lpD2dDevCtx, mgGetImage("Px-ListFocus"), rcDest);
	CListBox::DrawDefText(pCtrl, rcDest);
}
$VOID _CBACK mgDrawComboItem(CWidget *pCtrl, _in FRect &rcDest){
	DrawBitmap(GE2d::lpD2dDevCtx, mgGetImage("Px-CmbItem"), rcDest);
	CListBox::DrawDefText(pCtrl, rcDest);
}
$VOID _CBACK mgDrawComboFocus(CWidget *pCtrl, _in FRect &rcDest){
...
$VOID _CBACK mgProcPortalGui(CWidget *pCtrl, DWORD MsgCode, WPARAM Param0, LPARAM Param1){
	...
	switch(pCtrl->Ident){
		case ID_BTN_Start:{
			...
			gMyAppPage = gMyMaster = new CMGMaster;
			break;
		}
		case ID_BTN_Edit:{
			delete gMyGuiRoot;
			...
			gMyAppPage = gMyEditor = new CMGEditor;
			break;
		}
		case ID_BTN_Quit:{
	...
	switch(pCtrl->Ident){
		case ID_BTN_Quit:{
			if(ConfirmMessage(L"Back to home?")){
				...
				gMyAppPage = gMyPortal = new CMGPortal;
			}
			break;
		}
	}
...
$VOID _CBACK mgProcEditorGui(CWidget *pCtrl, DWORD MsgCode, WPARAM Param0, LPARAM Param1){
	static WORDT tTerrMode = CMGEditor::MODE_Uplift;
	static WORDT tBrushFunc = CMGEditor::MODE_EditColor;

	auto pfnPainting = []()->IBOOL{
		if(gMyEditor->FuncMode == CMGEditor::MODE_EditColor)
			return B_True;
		if(gMyEditor->FuncMode == CMGEditor::MODE_EditPattern)
			return B_True;
	...
	switch(pCtrl->Ident){
		...
		case ID_BTN_BrushSize:{
			...
			gMyEditor->SetBrushSize(_wtoi(psValue));
			mgGetLabel(ID_LBL_BrushSize)->SetValue(psValue);
			pfnPrompt(IDS_Hint8);
		} break;
		case ID_BTN_BrushPower:{
			const WCHAR *psValue = pCtrl->GetValue();
			gMyEditor->SetBrushPower((SPFPN)_wtof(psValue));
			mgGetLabel(ID_LBL_BrushPower)->SetValue(psValue);
			pfnPrompt(IDS_Hint9);
		} break;
		case ID_RDO_RectBrush:{
			gMyEditor->SetBrushShape(CMGEditor::BR_TYPE_Rect);
		} break;
		case ID_RDO_CircleBrush:{
			gMyEditor->SetBrushShape(CMGEditor::BR_TYPE_Circle);
		} break;
		case ID_RDO_RhombusBrush:{
			gMyEditor->SetBrushShape(CMGEditor::BR_TYPE_Rhombus);
		} break;
		case ID_RDO_Uplift:{
			pfnPrompt(IDS_Hint1);
			pfnUnselect(ID_GRP_SculptScape);
			tTerrMode = CMGEditor::MODE_Uplift;
			gMyEditor->SetFunctionMode(CMGEditor::MODE_Uplift);
			mgGetLabel(ID_LBL_SculptScape)->SetValue(mgGetText(IDS_Uplift));
		} break;
		case ID_RDO_Steepen:{
			pfnPrompt(IDS_Hint2);
			pfnUnselect(ID_GRP_SculptScape);
			tTerrMode = CMGEditor::MODE_Steepen;
			gMyEditor->SetFunctionMode(CMGEditor::MODE_Steepen);
			mgGetLabel(ID_LBL_SculptScape)->SetValue(mgGetText(IDS_Steepen));
		} break;
		case ID_RDO_Flatten:{
			pfnPrompt(IDS_Hint3);
			pfnUnselect(ID_GRP_SculptScape);
			tTerrMode = CMGEditor::MODE_Uniform;
			gMyEditor->SetFunctionMode(CMGEditor::MODE_Uniform);
			mgGetLabel(ID_LBL_SculptScape)->SetValue(mgGetText(IDS_Uniform));
		} break;
		case ID_RDO_Sharpen:{
			pfnPrompt(IDS_Hint4);
			pfnUnselect(ID_GRP_SculptScape);
			tTerrMode = CMGEditor::MODE_Sharpen;
			gMyEditor->SetFunctionMode(CMGEditor::MODE_Sharpen);
			mgGetLabel(ID_LBL_SculptScape)->SetValue(mgGetText(IDS_Sharpen));
		} break;
		case ID_RDO_Wiring:{
			pfnPrompt(IDS_Hint5);
			pfnUnselect(ID_GRP_SculptScape);
			tTerrMode = CMGEditor::MODE_Wiring;
			gMyEditor->SetFunctionMode(CMGEditor::MODE_Wiring);
		...
		case ID_LST_BrushColor:{
			...
			if(!pItem) break;
			pMutex->pFocusItem = P_Null;
			tBrushFunc = CMGEditor::MODE_EditColor;
			if(pfnPainting()) gMyEditor->SetFunctionMode(tBrushFunc);
		...
		case ID_LST_Decorat:{
			...
			switch(ObjType){
				case UINT_MAX:
				case 0:{
					gMyEditor->SetFunctionMode(CMGEditor::MODE_MakeRoad);
				} break;
				default:{
					gMyEditor->SetFunctionMode(CMGEditor::MODE_MakeDecor);
		...
		case ID_LST_Actor:{
			DWORD ObjId = ((CListBox*)pCtrl)->GetChoice();
			gMyEditor->SetFunctionMode(CMGEditor::MODE_AddUnit);
			gMyEditor->SetPreviewObject(ObjId);
		} break;
		...
		case ID_CMB_DecorType:{
			...
			for(; pItem; pItem = pItem->GetNext()){
				UNS32 Ident = StrHash32(pItem->GetValS());
			...
			switch(pCmbBox->GetChoice()){
				...
				case IDS_DecorModule:{
					...
					if(!gMyEditor->InSceneSetupMode())
						gMyEditor->SetFunctionMode(CMGEditor::MODE_None);
				} break;
				case IDS_ActorModule:{
					...
					if(!gMyEditor->InActorSetupMode())
						gMyEditor->SetFunctionMode(CMGEditor::MODE_AdjustActor);
				} break;
			}
		} break;
	}
...
CPanel *mgInitPortalGui(){
	...
	CLabel::XDesc LblDesc;
	CButton::XDesc BtnDesc;

	UNS32 rgCtlId[] = { ID_BTN_Start, ID_BTN_Edit, ID_BTN_Quit };
	UNS32 rgTxtId[] = { IDS_StartGame, IDS_EditScene, IDS_QuitApp };
	...
	for(UNS32 iCtl = 0; iCtl < NumCtrl; ++iCtl){
...
CPanel *mgInitMasterGui(){
	...
	auto pfnInitCmdBar = [&](SPFPN MaxX)->SPFPN{
		...
		//按钮
		{
			...
			CButton::XDesc CtlDesc = { 0, BasePos, BasePos, CtlSize, CtlSize,
			...
			for(UNS32 iRow = 0; iRow < 4; ++iRow){
				for(UNS32 iCol = 0; iCol < 4; ++iCol){
	...
	auto pfnInitItemBar = [&](SPFPN MaxX)->SPFPN{
		...
		//物品按钮
		{
			CButton::XDesc CtlDesc = { 0, Dist0, Dist1, CtlSize, CtlSize,
				1, BFM_Fill, P_Null, mgDrawButton2, P_Null };
...
CPanel *mgInitEditorGui(){
	...
	SPFPN EdgeDistX = 12.f;
	CPanel *pTopPanel = P_Null;
	IDWriteTextFormat *pTxtFmt = P_Null;

	auto pfnInitFileMenu = [&](SPFPN MinY)->SPFPN{
		//按钮

		UNS32 rgTxtId[] = { IDS_New, IDS_Open, IDS_Quit, IDS_Save, IDS_SaveAs, IDS_Set };
		UNS32 rgCtlId[] = { ID_BTN_New, ID_BTN_Open, ID_BTN_Quit, ID_BTN_Save, ID_BTN_SaveAs, ID_BTN_Set };

		UNS32 NumCtrl = ARR_LEN(rgCtlId);
		UNS32 NumCol = 3;
		...
		for(UNS32 iCol = 0, iCtl = 0; iCtl < NumCtrl; ++iCtl){
			if(iCol++ == NumCol){
				iCol = 0;
				BtnDesc.PosX = BaseX;
	...
	auto pfnInitToolList = [&](SPFPN MinY)->SPFPN{
		UNS32 rgTxtId[] = { IDS_TerrModule, IDS_DecorModule, IDS_ActorModule };
		UNS32 NumText = ARR_LEN(rgTxtId);

		CComboBox *pComboBox;
		CLabel::XDesc LblDesc;
		CButton::XDesc BtnDesc;
		CListBox::XDesc ListDesc;
		CComboBox::XDesc CmbDesc;
	...
	auto pfnInitTerrTool = [&](SPFPN MinY){
		...
		//标签
		{
			...
			for(UNS32 iCtl = 0; iCtl < ARR_LEN(rgTxtId); ++iCtl){
				LblDesc.psText = mgGetText(rgTxtId[iCtl]);
		...
		//按钮组
		{
			...
			for(UNS32 iSet = 0; iSet < ARR_LEN(rgGrpId); ++iSet){
		...
		//按钮
		{
			UNS32 rgCtlId[2] = { ID_BTN_BrushSize, ID_BTN_BrushPower };
			WCHAR *rgCtlVal[2][5] = {
				{ L"2", L"4", L"8", L"16", L"32" },
				{ L"0.1", L"0.2", L"0.5", L"0.8", L"1" } };

			BtnDesc.PosY = RowDimY * 7.f;
			LblDesc.AliMode = POS_Center;

			for(UNS32 iRow = 0; iRow < 2; ++iRow){
				BtnDesc.PosX = EdgeDistX;
				BtnDesc.PosY += RowDimY * 2.f;
				BtnDesc.Ident = rgCtlId[iRow];

				for(UNS32 iCol = 0; iCol < 5; ++iCol){
		...
		//颜色列表
		{
			...
			UNS32 NumItem = pItems->GetSize();

			for(UNS32 iItem = 0; iItem < NumItem; ++iItem){
				const CHAR8 *psValue = pItems->GetElemS(iItem);
				pListBox->AddColorItem(strtoul(psValue, P_Null, 16) | 0xFF000000);
			}
		}
		...
		//纹理列表
		{
			...
			for(UNS32 iItem = 0; iItem < pSheet->GetFrameCount(); ++iItem)
	...
	auto pfnInitDecorTool = [&](SPFPN MinY){
		...
		//
		{
			...
			for(UNS32 iTxt = MIN_SCENE_ID; iTxt <= MAX_SCENE_ID; ++iTxt)
	...
	auto pfnInitActorTool = [&](SPFPN MinY){
		...
		//
		{
			...
			for(UNS32 iTxt = MIN_TEAM_ID; iTxt <= MAX_TEAM_ID; ++iTxt)
				pComboBox->AddItem(mgGetText(iTxt));
		}

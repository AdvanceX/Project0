...
inline POINT3 RectCenter(_in RECTF &Rect){
	SPFPN X = (Rect.left + Rect.right) * 0.5f;
	SPFPN Z = (Rect.bottom + Rect.top) * 0.5f;
	return POINT3(X, 0.f, Z);
}
inline POINT3 RectCenter(_in RECTU &Rect){
	SPFPN X = (Rect.left + Rect.right) * 0.5f;
	SPFPN Z = (Rect.bottom + Rect.top) * 0.5f;
	return POINT3(X, 0.f, Z);
}
